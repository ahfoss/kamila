...
#' @return A vector of variable-specific weights with length equal to \code{ncol(dat)}.
medeaWgts <- function(
  dat,
  ...
  if (associationMethod == 'ari') {
    ...
    # generate matrix using multiple correspondence analysis
    famdRes <- FAMD(dat, ncp=ncol(dat), graph=FALSE)$var$contrib
    wgts <- famdRes[,1]
    wgts <- wgts / sum(wgts)
    results <- list(wgts=wgts)
  }
